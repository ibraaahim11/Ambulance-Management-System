...
private:
	...
	int NumCheckupCars, TotalNumChecked;
...
public:
	...
	float GetFailedPercentage() {
		float percentage = ((float)TotalNumChecked / (float)(TotalNumNC + TotalNumSC));
	...
	void OutputFile(int timestep) {
		...
		if (Output.is_open()) {
			...
			Output << "Failed Cars= " << TotalNumChecked << ", Percentage: " << setprecision(4) << GetFailedPercentage() << "%" << endl;
		}
		else {
